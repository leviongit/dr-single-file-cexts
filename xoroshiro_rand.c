...
struct xoroshiro128pp_st {
  uint64_t lo;
  uint64_t hi;
};

static_assert(sizeof(struct xoroshiro128pp_st) <= ISTRUCT_DATA_SIZE);

uint64_t xoroshiro128pp_next(struct xoroshiro128pp_st *st) {
...
void xoroshiro128pp_jump(struct xoroshiro128pp_st *st) {
...
void xoroshiro128pp_long_jump(struct xoroshiro128pp_st *st) {
...
void xoroshiro128pp_init(struct xoroshiro128pp_st *st, uint64_t seed) {
  *st = (struct xoroshiro128pp_st){
      .hi = rotl(seed ^ UINT64_C(0xfac1e04741dab55a), seed & 0x1f),
      .lo = rotl(seed, 12) ^ UINT64_C(0xf01e46382d57cab9)};
}

double xoroshiro128pp_next_float(struct xoroshiro128pp_st *st) {
  return (xoroshiro128pp_next(st) & ~UINT64_C(1)) / (double)UINT64_MAX;
}

_Bool xoroshiro128pp_next_bool(struct xoroshiro128pp_st *st) {
  return xoroshiro128pp_next(st) >> 63;
}

struct RClass *xoroshiro128pp;

mrb_value xoro_rand_alloc(mrb_state *mrb, mrb_value) {
  struct RIStruct *ris =
      (struct RIStruct *)mrb_obj_alloc(mrb, MRB_TT_ISTRUCT, xoroshiro128pp);
...
mrb_value xoro_rand_new(mrb_state *mrb, mrb_value) {
  ...
  xoroshiro128pp_init((struct xoroshiro128pp_st *)&ris->inline_data, seed);

  return val;
}

mrb_value xoro_rand_init(mrb_state *mrb, mrb_value self) {
  if (!mrb_obj_is_instance_of(mrb, self, xoroshiro128pp)) {
    mrb_raisef(mrb, E_TYPE_ERROR, "%C::initialize called on %T", xoroshiro128pp,
               self);
  }
  ...
  xoroshiro128pp_init((struct xoroshiro128pp_st *)ISTRUCT_PTR(self), seed);
...
mrb_value xoro_rand_rand(mrb_state *mrb, mrb_value self) {
  ...
  struct xoroshiro128pp_st *st = (typeof(st))ISTRUCT_PTR(self);
  ...
  if (mrb_integer_p(arg)) {
    ...
    if (i == 0)
      return mrb_float_value(mrb, xoroshiro128pp_next_float(st));

    return mrb_int_value(mrb, xoroshiro128pp_next(st) % i);
  ...
  if (mrb_range_p(arg)) {
    ...
    if (a_int_p && b_int_p) {
      ...
      mrb_int offs = xoroshiro128pp_next(st) % span;
    ...
    return mrb_float_value(mrb, fa + (xoroshiro128pp_next_float(st) * span));
  }
...
mrb_value xoro_rand_rand_bool([[maybe_unused]] mrb_state *mrb, mrb_value self) {
  return mrb_bool_value(
      xoroshiro128pp_next_bool((struct xoroshiro128pp_st *)ISTRUCT_PTR(self)));
}

void drb_register_c_extensions_with_api(mrb_state *mrb, void *) {
  xoroshiro128pp = mrb_define_class_id(mrb, mrb_intern_lit(mrb, "Xoroshiro128"),
                                       mrb->object_class);
  mrb_define_class_method_id(mrb, xoroshiro128pp,
                             mrb_intern_lit(mrb, "allocate"), xoro_rand_alloc,
                             MRB_ARGS_NONE());
  mrb_define_class_method_id(mrb, xoroshiro128pp, mrb_intern_lit(mrb, "new"),
                             xoro_rand_new, MRB_ARGS_OPT(1));
  mrb_define_method_id(mrb, xoroshiro128pp, mrb_intern_lit(mrb, "initialize"),
                       xoro_rand_init, MRB_ARGS_OPT(1));

  mrb_define_method_id(mrb, xoroshiro128pp, mrb_intern_lit(mrb, "rand"),
                       xoro_rand_rand, MRB_ARGS_OPT(1));
  mrb_define_method_id(mrb, xoroshiro128pp, mrb_intern_lit(mrb, "rand_bool"),
                       xoro_rand_rand_bool, MRB_ARGS_NONE());

  MRB_SET_INSTANCE_TT(xoroshiro128pp, MRB_TT_ISTRUCT);
}
